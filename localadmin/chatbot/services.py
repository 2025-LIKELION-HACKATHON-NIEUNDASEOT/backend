...
class GeminiChatService:
    ...
    def _build_prompt(
        ...
        '~에 대해서 알려드릴게요.' 라는 말도 하지말고 그냥 바로 본론으로 들어가서 답변하세요. 
        사용자의 개인 정보를 맨 첫 줄에 읊지 마세요."""
        ]
        # 사용자 정보
    ...
    def summarize_response(self, response: str, max_length: int = 65) -> str:
        ...
        try:
            prompt = (
                f"다음 텍스트의 핵심 내용을 {max_length}자 이내로 짧게 요약해주세요. 그것보다 더 짧아도 됩니다."
                f"문장을 명사로 끝내세요. 예를 들면 '정의' 나 '가능'과 같이 요약하세요. 무작정 대화 내용을 나열하지 마세요. 맨 끝에 '.'와 같은 점을 찍지 마세요.\n\n"
                f"예시: '용적률의 의미' 또는 '도봉구에서 열리는 콘서트는 누구나 참여 가능'\n\n"
                f"{response}\n\n"
                "요약:"
            )
