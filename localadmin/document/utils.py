...
def analyze_document_content(document_text, max_retries=3, base_delay=60):
    ...
    # 프롬프트
    prompt = f"""다음 공문을 분석하여 JSON 형식으로 추출해 주세요.
    결과는 어떠한 설명이나 추가 문장 없이 오직 JSON 객체만 포함해야 합니다.
    키워드는 반드시 공공문서에서 자주 쓰이는 '스포츠', '청년', '복지', '환경', '재정', '교육', '중소기업', '건강', '문화', '안전', '산업', '농업', '에너지', '주택', '법', '관광', '과학', '기술', '국방', '안전', '청소년', '교통'과 같은 일반적인 상위 개념으로 추출해 주세요.
    모든 키(title, keywords, related_departments, purpose, issue_date, summary)는 반드시 포함해야 하며,
    값이 없는 경우 빈 문자열("") 또는 빈 리스트([])로 처리해 주세요.
